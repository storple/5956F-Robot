...
void Autonomous::Skills() 
{
	...
	pros::delay(300);
	subsystem.intake.toggle();
	chassis.turnToPoint(-48, 72, 750, {.maxSpeed=70});
	chassis.moveToPoint(-48, 72, 2000, {.maxSpeed=75}, false);
	chassis.turnToPoint(-58, 48, 750, {.maxSpeed=80});
	chassis.moveToPoint(-58, 48, 1500, {.maxSpeed=40});
	pros::delay(500);
	subsystem.arm.PID(3800, 750);
	chassis.moveToPose(-52, 52, -90, 850, {.forwards=false, .lead=0.75, .maxSpeed=80});
	chassis.turnToPoint(-72, 48, 500, {.maxSpeed=100});
	chassis.moveToPoint(-72, 48, 2000, {.maxSpeed=30});
	pros::delay(400);
	subsystem.intake.toggle();
	pros::delay(350);
	...
	ArmMotor2.move(-20);
	subsystem.intake.toggle();
	chassis.turnToPoint(-50, -12, 750, {.maxSpeed=80});
	chassis.moveToPoint(-50, -12, 2000, {.maxSpeed=60});
	// chassis.waitUntilDone();
	// chassis.setPose(-48, -7.5, 180);
	chassis.turnToPoint(-60, 2, 750, {.maxSpeed=80});
	chassis.moveToPoint(-60, 2, 1500, {.maxSpeed=70});
	// chassis.turnToPoint(-64, -12, 750, {.forwards=false, .maxSpeed=80});
	chassis.moveToPoint(-64, -12, 1100, {.forwards=false, .maxSpeed=60}, false);
	subsystem.intake.toggle();
	...
	chassis.turnToPoint(48, 72, 750, {.maxSpeed=80});
	chassis.moveToPoint(48, 72, 2000, {.maxSpeed=75}, false);
	chassis.turnToPoint(58, 48, 750, {.maxSpeed=80});
	chassis.moveToPoint(58, 48, 1500, {.maxSpeed=40});
	pros::delay(500);
	...
	// chassis.turnToPoint(64, -12, 750, {.forwards=false, .maxSpeed=80});
	chassis.moveToPoint(64, -12, 900, {.forwards=false, .maxSpeed=60});
	chassis.waitUntilDone();
	subsystem.intake.toggle();
	subsystem.pneumatics.toggleLatch();
	pros::delay(250);
	...
	chassis.turnToPoint(18, 78, 500, {.maxSpeed=80});
	chassis.moveToPoint(18, 78, 1500, {.maxSpeed=80});
    pros::delay(350);
    subsystem.intake.toggle();
    LimitSwitchIntake(1000);
    
    chassis.turnToPoint(0, 98, 1000, {.forwards=false, .maxSpeed=80});
    ClampPID(10,1750,false);
    pros::delay(250);
    subsystem.intake.toggle();
	pros::delay(500);
    ...
    pros::delay(500);
    subsystem.intake.toggle();
    chassis.turnToPoint(-49, 95, 750, {.maxSpeed=80}, false);
	subsystem.pneumatics.toggleDoinker();
    chassis.moveToPoint(-49, 95, 1500, {.maxSpeed=100});
	pros::delay(1000);
	subsystem.intake.toggle();
	chassis.waitUntilDone();
	// chassis.moveToPoint(-48, 108, 1000, {.maxSpeed=80});
    // chassis.turnToPoint(-60, 96, 750, {.maxSpeed=80});
    // chassis.moveToPoint(-60, 96, 1500, {.maxSpeed=80});
    ...
    chassis.moveToPoint(60, 108, 1800, {}, false);
	subsystem.arm.PID(15400, 500);
	chassis.moveToPose(30, 78, 45, 1150, {.forwards = false,.minSpeed=80});
