...
class Temperature : public Quantity<std::ratio<0>, std::ratio<0>, std::ratio<0>, std::ratio<0>, std::ratio<0>,
    ...
    public:
        explicit constexpr Temperature(float value)
...
template <> struct std::formatter<Temperature> : std::formatter<float> {
...
constexpr Temperature operator""_kelvin(long double value) { return Temperature(static_cast<float>(value)); }

constexpr Temperature operator""_kelvin(unsigned long long value) { return Temperature(static_cast<float>(value)); }

constexpr Temperature operator""_celsius(long double value) { return Temperature(static_cast<float>(value) + 273.15); }

constexpr Temperature operator""_celsius(unsigned long long value) {
    return Temperature(static_cast<float>(value) + 273.15);
}

constexpr Temperature operator""_fahrenheit(long double value) {
    return Temperature((static_cast<float>(value) - 32) * (5.0 / 9.0) + 273.5);
}
...
constexpr inline float to_kelvin(Temperature quantity) { return quantity.internal(); }

constexpr inline Temperature from_celsius(Number value) { return Temperature(value.internal() + 273.15); }

constexpr inline float to_celsius(Temperature quantity) { return quantity.internal() - 273.15; }

constexpr inline Temperature from_fahrenheit(Number value) {
    return Temperature((value.internal() - 32) * (5.0 / 9.0) + 273.15);
}

constexpr inline float to_fahrenheit(Temperature quantity) {
